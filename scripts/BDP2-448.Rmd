...
After crossover, there is a slightly lower normalized food intake among the mice under bedding type A.
...
Before crossover, there was virtual no difference between normalized food intake by bedding type:
`r betaB %>% pull(estimate) %>% sprintf("%.3g", .)` mg/g
(p-value = `r betaB %>% pull(statistic) %>% pt(df = 1, lower.tail = FALSE) %>% sprintf("%.2g", .)`)
